 ...
 * for your use.
 */
static pcb_t **current, *tail, *head;
static pthread_mutex_t current_mutex, ready_q_mutex;
static int priority_mode, time_slice, cpu_count;
static pthread_cond_t idle_cond;

extern void add(pcb_t *process)
{
		...
		tail = process;
			...
			if(process->static_priority > head->static_priority){
				...
				head = process;
					...
					i = head;
					if(process->static_priority < tail->static_priority){
						tail -> next = process;
						tail = process;
						}
						else{
							while(temp->next && temp->next->static_priority){
								 i = i->next;
								 }
								 process->next = i->next;
								 i->next = process;
...
static void schedule(unsigned int cpu_id)
{
	...
	context_switch(cpu_id, current[cpu_id], time_slice);
...
extern void idle(unsigned int cpu_id)
{
	...
	while(peek() == NULL)
	{
			pthread_cond_wait(&idle_cond, &ready_q_mutex);
		}
...
extern void wake_up(pcb_t *process)
{
	...
	for(k = 0; k < cpu_count; k++)
	{
		...
		if(process->static_priority > current[i]->static_priority))
		{
			pthread_mutex_unlock(&current_mutex);
			force_preempt(i);
			break;
